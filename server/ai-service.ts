...
export interface AIAnalysisRequest {
  ...
  prompt: string;
  fileContext?: string;
  analysisType: 'qa' | 'grammar' | 'paraphrase' | 'summary' | 'citation' | 'literature';
}
...
Provide helpful, accurate, and academically appropriate responses. Be concise but thorough.`;
  ...
  private getSystemPrompt(analysisType: string): string {
    const prompts = {
      ...
      grammar: "You are an academic writing expert. Check the provided text for grammar, style, clarity, and academic tone. Provide specific suggestions with explanations.",
      
      paraphrase: "You are a writing assistant. Paraphrase the provided text while maintaining its academic meaning and improving clarity. Preserve technical terms and key concepts.",
      
      summary: "You are a research analyst. Create a concise, well-structured summary that captures the main arguments, findings, and conclusions of the text.",
      
      citation: "You are a citation expert. Generate proper academic citations in the requested format (APA, MLA, or Chicago) based on the provided information.",
      
      literature: "You are a research librarian. Suggest relevant academic papers and research directions based on the provided text and research focus."
    };
  ...
  async extractTextContent(fileName: string, mimeType: string, buffer: Buffer): Promise<string> {
